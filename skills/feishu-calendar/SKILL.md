...
> "建一个每周一到周五 9 点的晨会"
> "创建每天 9 点测试重复日程，提前 15 分钟提醒，参会人加我自己"
> "明天上午找个时间提醒我处理报销，提前 30 分钟提醒"
> "下周三 14 点开会，提前 1 小时提醒"
...
5. 若用户提到“提前 X 分钟/小时提醒”，必须显式构造 `reminders`
   - 提前 5 分钟 → `[{"minutes":5}]`
   - 提前 15 分钟 → `[{"minutes":15}]`
   - 提前 30 分钟 → `[{"minutes":30}]`
   - 提前 1 小时 → `[{"minutes":60}]`
   - 多个提醒可传数组，如 `[{"minutes":60},{"minutes":15}]`
6. 若用户提到“每天/每周/工作日/直到某天/共 N 次”，必须优先显式构造 `recurrence`，不要把重复规则留给用户手动设置
...
- 需要提醒时，不要只在文案里说“已设置提醒”，而要显式传 `reminders`
- “提前 X 分钟/小时提醒”不限具体时间，任何日期/时间场景都应映射为对应 `minutes`
- 常见映射：5→5，10→10，15→15，30→30，1小时→60，2小时→120
