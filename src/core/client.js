  ...
  for (let i = 0; i <= retries; i++) {
    try {
      ...
      if (res.status === 429 || res.status >= 500) {
        ...
        lastErr = new FeishuPlusError("feishu_retryable_http", `HTTP ${res.status}`, {
          status: res.status,
          body,
        });
        if (i < retries) {
  ...
  return {
    ...
    async bitableBatchCreateRecords(appToken, tableId, records) {
      return call(
        ...
        {
          method: "POST",
          body: { records: records.map((r) => ({ fields: r.fields })) },
        },
      );
    },
    async bitableBatchUpdateRecords(appToken, tableId, records) {
      return call(
        ...
        {
          method: "POST",
          body: { records: records.map((r) => ({ record_id: r.record_id, fields: r.fields })) },
        },
      );
    },
